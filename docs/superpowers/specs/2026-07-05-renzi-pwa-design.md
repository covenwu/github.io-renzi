...
## 11. 未来扩展备忘（已评估可行，本期不实现）

### 组词联动推送

利用开源二字词表（THUOCL/常用词表截取，~100–200KB，作为静态资源随代码缓存，
不入数据库）辅助记忆，分两级：

1. **纯展示**：字卡作答后展示该字与字库内其他字组成的词语（零调度影响）；
2. **相邻推送**：取卡时若当前字与**今日到期队列内**另一字可组词，
   将搭档字提到紧随其后按词序出现；多个候选时选逾期最久者。
   仅在到期字之间配对，严格不违反「不提前复习」原则。

原则：**先字后词**——第二个字作答前不提示组词关系，作答后才展示词语，
保证每个字的记忆测试独立、调度证据不掺水。

### 复习会话前进/后退

会话内维护已出示卡片的历史列表，可回翻查看。已作答卡片为**只读回看**
（显示字/照片/发音/词语/作答结果，无作答按钮），仅队列前端当前卡可作答；
回看不写库、不影响调度。配合组词推送实现「见词回看前字」。
可选后续：「撤销最近一次作答」（Anki 式 undo），观察实际需要后再加。
